#!/usr/bin/env python3
"""
Optimized Discrete-Time Simulation — NumPy arrays + Numba JIT.

The public simulate() function has an identical signature and return value
...
------------
  Pre-processing  (Python)
    All factory objects (workstations, configurations, BOM, layout) are
    converted to flat NumPy integer/float arrays.  String IDs are replaced
    by integer indices.  The BOM is stored in CSR (Compressed Sparse Row)
    format.  Every demand is pre-allocated in fixed-size arrays.

  Tick loop  (Numba @njit — compiled to machine code on first call)
    _numba_tick_loop() receives only arrays and scalars; no Python objects,
    dicts, or strings enter the compiled function.  Numba compiles it to
    LLVM machine code (~1–2 s on first call; cached to disk for later runs).

  Post-processing  (Python + NumPy/Pandas)
    Raw output arrays are converted back to the same DataFrames the initial
    model produces.  This step stays in Python because Pandas is not
    supported inside @njit.
...
_STATE_NAMES = ["idle", "setup", "processing", "blocked", "starved"]
...
def _numba_tick_loop(
    ...
    for tick in range(n_ticks):
        ...
        # Process level by level (lowest first) so sub-assemblies are always
        # produced before the assemblies that need them.
        for lvl in range(1, max_level + 1):
            for di in range(n_demands):
                ...
                # Find the capable idle/starved workstation with lowest ETA.
                # ETA accounts for a possible setup if the component changes.
                best_wi  = -1
                ...
                if best_wi < 0:
                    continue   # no capable workstation free

                # Consume non-raw BOM inputs from stock
                for k in range(bom_ptr[ci], bom_ptr[ci + 1]):
                    inp_ci  = bom_inputs[k]
                    inp_qty = bom_qtys[k]
        ...
        #       demand they cannot start due to missing input stock.
        for wi in range(n_ws):
            if ws_state[wi] != _IDLE and ws_state[wi] != _STARVED:
                continue
            starved = False
...
def simulate(
    ...
    Identical signature and return value to the initial model's simulate().
    Internally uses a Numba-compiled tick loop for speed.
    ...
    log_buffers         When False, the 'buffers' DataFrame is empty.
                        Disable in sweep runs to keep memory usage low.

    Returns
    -------
    dict with five DataFrames: 'states', 'utilization', 'throughput',
    'costs', 'buffers'  — same schema as the initial model.
    """
    ...
    # Every component and workstation gets a stable integer index so the
    # Numba loop can use array indexing instead of dict/string lookups.
    ...
    # Stock: raw materials (level 0) = effectively infinite; others start at 0.
    _INF = 10_000_000
    stock = np.array(
    ...
    # ── BOM in CSR format ──────────────────────────────────────────────────────
    # For each component ci, bom_ptr[ci]:bom_ptr[ci+1] indexes into
    # bom_inputs_flat / bom_qtys_flat to give its direct input components.
    ...
    # ── Configuration matrices [n_ws, n_comps] ────────────────────────────────
    # Dense 2-D arrays indexed by (workstation_index, component_index).
    # capable[wi, ci] is True when workstation wi can produce component ci.
    ...
    # Average cost of all layout edges arriving at each workstation.
    ...
    # Doing this in Python once avoids re-exploding every order release
    # inside the (Numba) tick loop, where dicts are not available.
    ...
    def _explode(prod_id: str) -> dict[str, int]:
        ...
        # Return only non-raw components (raw materials are always available)
    ...
    ws_current_comp = np.full(n_ws, -1,    dtype=np.int32)  # -1 = not set up
    ws_job_demand   = np.full(n_ws, -1,    dtype=np.int32)  # -1 = no job
    ws_job_phase    = np.zeros(n_ws,       dtype=np.int8)
    ws_job_qty      = np.zeros(n_ws,       dtype=np.int32)

    # ── Demand queue pre-allocation ────────────────────────────────────────────
    # Worst case: every order needs every non-raw component.
    ...
    # ── Output log arrays (pre-allocated at maximum possible size) ─────────────
    state_log = np.zeros((n_ticks, n_ws),   dtype=np.int8)
    tp_log    = np.zeros((n_orders, 5),     dtype=np.float64)
    # buf_log: allocate a minimal stub when not logging to save memory.
    ...
    # States: vectorised construction (much faster than list-of-dicts for
    # large simulations).
...
if __name__ == "__main__":
    ...
    print("Running optimized discrete-time simulation…")
    results = simulate(
        gen_result,
