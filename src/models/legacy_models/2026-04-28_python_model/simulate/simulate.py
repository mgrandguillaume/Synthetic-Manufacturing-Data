...
simultaneously.  This enables three phenomena absent in a purely sequential
scheduler:
...
def simulate(
    ...
    Parameters
    ----------
    gen_result          Output of generate_simple_assembly().
    ...
    # Average transport cost per workstation (over all incoming layout edges).
    ...
    def _explode(prod: str) -> dict[str, int]:
        """
        Return the total units of every component required for one unit of prod.
        Walks top-down level by level so shared sub-assemblies accumulate correctly.
        """
    ...
    for tick in range(n_ticks):
        ...
        for ws in ws_map.values():
            ...
            if ws.job.phase == "setup":
                ...
                ws.job.ticks_left = _ticks(ws.job.cfg["proc_time"] * ws.job.demand.qty)
                ws.state          = "processing"
            else:
        ...
        # 4. Assign idle workstations to unmet demand, lowest BOM level first.
        pending = sorted(
        ...
        unassigned = [d for d in demand_queue if not d.assigned and not d.fulfilled]
        for ws in ws_map.values():
            if ws.state not in ("idle", "starved"):
                continue
            starved = any(
                any(cfg["ws"] == ws.ws_id for cfg in comp_configs.get(d.component, []))
        ...
        # 7. Log component buffer levels for this tick (optional).
        if log_buffers:
            for c in components:
                if c.level > 0:
                    buffer_rows.append({
...
if __name__ == "__main__":
    ...
    _SIM_DIR     = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sim_output")
    ...
    for name, df in results.items():
        path = os.path.join(_SIM_DIR, f"{name}.csv")
