...
def _build_factory(
    ...
    """
    Build a factory from scalar parameters.  Called by both
    generate_simple_assembly() and generate_from_params().
    """
    ...
    assert topology in ("parallel", "linear"), \
        "layout.topology must be 'parallel' or 'linear'"
...
def _write_csvs(result: dict, out_dir: str) -> None:
    ...
    with open(os.path.join(out_dir, "configurations.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["ID", "Workstation", "Component", "ProcessingTime",
                    "SetupTime", "SetupCost", "OperatingCost"])
...
def generate_from_params(params: dict, export_csv: bool = False,
    ...
    Optional keys
    -------------
    topology  ("parallel" | "linear", default "parallel")
    seed      (int | None)
    """
...
def generate_simple_assembly(config_path: str, export_csv: bool = True) -> dict:
    ...
    Intended for standalone / single-run use.

    Parameters
    ----------
    config_path : absolute or relative path to config.yaml
    export_csv  : when True, writes CSVs to output.directory from the config
    """
    ...
    params = {
        ...
        "depth":                   bom_cfg["depth"],
        "branching":               bom_cfg["branching"],
        "quantity":                bom_cfg["quantity"],
        "sharing_ratio":           bom_cfg.get("sharing_ratio", 0.0),
        "workstations_count":      cfg["workstations"]["count"],
        "producers_per_component": ccfg["producers_per_component"],
        "processing_time":         ccfg["processing_time"],
        "setup_time":              ccfg["setup_time"],
        "setup_cost":              ccfg["setup_cost"],
        "operating_cost":          ccfg["operating_cost"],
    ...
    if export_csv:
        ...
        out_dir    = rel if os.path.isabs(rel) else os.path.normpath(
            os.path.join(script_dir, "generate", rel))
